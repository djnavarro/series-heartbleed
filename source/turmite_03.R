...
walk_time <- 500000
palette <- "tokyo"
filename <- "~/Desktop/turmite_03.png"
...
machine$grid <- Matrix(machine$grid, sparse=TRUE)
...
transition <- function(machine, time) {
  ...
  if(colour == 0 & turmite$state == 1) {
    ...
    turmite <- turn(turmite, "none")      # turn
    turmite <- move(turmite)              # move
    turmite$state <- 0
    return(list(grid = grid, turmite = turmite))
  }
...
machine$grid <- as.matrix(machine$grid)

png(filename, 1000, 1000)
op <- par(mar=c(0,0,0,0))
image(machine$grid, axes = FALSE, col = scico::scico(5000, palette = palette))
dev.off()
par(op)
