...
output <- here::here("turmite_28.png")
set.seed(12)
...
dat <- use_seed(83) %>%
  entity_circle(grain = 1000, size = .25) %>%
  unfold_slice(iterations = 7, scale = 10^-24, scatter = TRUE, output1 = "id") %>%
  mutate(
    x = x + rnorm(n())/500,
    y = y + rnorm(n())/500
  ) %>%
...
f <- scico::scico
palette_heart <- palette <- paletteer::paletteer_c("scico::lajolla", ncols + 1)
