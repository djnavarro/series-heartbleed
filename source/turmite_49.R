...
output <- here::here("turmite_49.png")
set.seed(500)
...
palette <- f(ncols + 1)
palette <- paletteer::paletteer_c("grDevices::Oranges", ncols + 1)
...
dat <- use_seed(500) %>%
  entity_heart(grain = 1000, size = .5) %>%
  mutate(ind = 1:n()) %>%
...
palette_heart <- paletteer::paletteer_c("grDevices::Oranges", ncols + 1)
...
pic <- ggplot(
  ...
  geom_point(
    data = dat, 
    mapping = aes(x,y, alpha = exp(-(time-1)/20)), 
    color = "white",
    inherit.aes = FALSE,
    show.legend = FALSE,
    size = .5
