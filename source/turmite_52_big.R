...
output <- here::here("turmite_52_big.png")

seed <- 503
ncols <- 1000

fill_base <- "ghostwhite"
palette_base <- paletteer::paletteer_c("grDevices::TealRose", ncols + 1)
palette_heart <- paletteer::paletteer_c("grDevices::TealRose", ncols + 1)
...
dat <- use_seed(seed) %>%
  ...
  mutate(ind = 1:n()) %>%
  unfold_slice(iterations = 8, scale = .5 * 10^-24, scatter = TRUE, output1 = "id") %>%
  mutate(
    x = x + rnorm(n())/500,
    y = y + rnorm(n())/500
  ) %>%
